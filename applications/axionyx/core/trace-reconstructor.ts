import { AuditLogEntry, Sequence, Fact } from './types';
 ...
 */
export function reconstructTrace(traceId: string, auditLog: AuditLogEntry[]): Sequence {
  // 1. Filter facts that belong exactly to this trace
  const traceFacts: Fact[] = auditLog.filter(entry => entry.traceId === traceId);
