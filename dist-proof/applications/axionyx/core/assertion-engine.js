...
const EvaluatorRegistry = {
    "actor-exists": (ctx) => {
        ...
        if (ctx.contextFrame.resolutionStatus === "ACTOR_NOT_FOUND")
            return "UNKNOWN";
        if (ctx.contextFrame.resolutionStatus === "INSUFFICIENT_HISTORY")
            return "UNKNOWN";
        if (ctx.contextFrame.resolutionStatus === "RESOLVED")
            return "PASS";
        return "UNKNOWN";
    },
    "trace-exists": (ctx) => {
    ...
    "required-antecedent": (ctx, params) => {
        ...
        if (currentIndex <= 0)
            return "FAIL"; // Cannot have an antecedent if it's the first node
    ...
    "actor-has-permission": (ctx, params) => {
        if (!params || !params.action || !params.resource)
            return "UNKNOWN";
        if (ctx.contextFrame.resolutionStatus !== "RESOLVED")
            return "UNKNOWN";
        const hasPerm = ctx.contextFrame.activePermissions.some(p => p.action === params.action && p.resource === params.resource);
    ...
    "chronology-valid": (ctx) => {
        ...
        for (const node of ctx.sequence.nodes) {
            ...
            if (time < lastTime)
                return "FAIL";
            lastTime = time;
            if (node.eventId === ctx.fact.eventId)
                break; // Only check up to current fact
        }
        return "PASS";
    },
    "no-duplicate-facts": (ctx) => {
        // Check if this fact appears more than once in the sequence
        const occurrences = ctx.sequence.nodes.filter(n => n.action === ctx.fact.action && n.targetResource === ctx.fact.targetResource && n.actorId === ctx.fact.actorId);
 ...
 */
function evaluateAssertion(assertionConfig, ctx) {
