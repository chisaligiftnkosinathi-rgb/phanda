 ...
 */
function buildContextFrame(fact, identitySnapshot, trustEvidence = []) {
    ...
    const activeMemberships = identitySnapshot.memberships.filter(m => {
        if (m.personId !== resolvedPersonId)
            return false;
        const validFromTime = new Date(m.validFrom).getTime();
        if (validFromTime > eventTime)
            return false; // Membership started after the event
        if (m.validTo) {
            const validToTime = new Date(m.validTo).getTime();
            if (validToTime < eventTime)
                return false; // Membership ended before the event
        }
        return true;
    });
    ...
    frame.activeTrustEvidence = trustEvidence.filter(e => {
        ...
        if (issuedTime > eventTime)
            return false;
        if (e.expiresAt) {
            const expiresTime = new Date(e.expiresAt).getTime();
            if (expiresTime < eventTime)
                return false;
        }
        return true;
    });
