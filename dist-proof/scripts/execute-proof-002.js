...
const interpret_1 = require("../applications/axionyx/api/interpret");
const TRACE_ID = "06a0500e-6994-4ad8-99b1-597374102f5a";
const DB_PATH = path_1.default.resolve(__dirname, '../dev.db');
const PROOFS_DIR = path_1.default.resolve(__dirname, '../proofs/Proof_of_Work_002');
const GOVERNANCE_FILE = path_1.default.resolve(__dirname, '../applications/axionyx/governance-v1.json');
...
function extractEvidence() {
    ...
    fs_1.default.writeFileSync(path_1.default.join(PROOFS_DIR, 'audit-log.json'), JSON.stringify(auditLog, null, 2));
    fs_1.default.writeFileSync(path_1.default.join(PROOFS_DIR, 'identity-snapshot.json'), JSON.stringify(identityGraph, null, 2));
    fs_1.default.writeFileSync(path_1.default.join(PROOFS_DIR, 'trust-evidence.json'), JSON.stringify([], null, 2));
    // Copy Governance
    fs_1.default.copyFileSync(GOVERNANCE_FILE, path_1.default.join(PROOFS_DIR, 'governance-v1.json'));
    const manifest = {
    ...
    fs_1.default.writeFileSync(path_1.default.join(PROOFS_DIR, 'manifest.json'), JSON.stringify(manifest, null, 2));
    console.log("✓ Evidence extracted and frozen.\n");
}
function runAxionyx() {
    const auditLog = JSON.parse(fs_1.default.readFileSync(path_1.default.join(PROOFS_DIR, 'audit-log.json'), 'utf8'));
    const identityGraph = JSON.parse(fs_1.default.readFileSync(path_1.default.join(PROOFS_DIR, 'identity-snapshot.json'), 'utf8'));
    const trustEvidence = JSON.parse(fs_1.default.readFileSync(path_1.default.join(PROOFS_DIR, 'trust-evidence.json'), 'utf8'));
    const request = {
        traceId: TRACE_ID,
        dataSource: {
            auditLog,
    ...
    return (0, interpret_1.analyzeTrace)(request);
}
...
function executePhaseB() {
    ...
    fs_1.default.writeFileSync(path_1.default.join(PROOFS_DIR, 'interpretation.json'), JSON.stringify(result.data.interpretation, null, 2));
...
function executePhaseD(baselineInterpretation) {
...
function testInvalidEvidence() {
    ...
    const result = (0, interpret_1.analyzeTrace)(badRequest);
    if (result.status === "INVALID_EVIDENCE") {
